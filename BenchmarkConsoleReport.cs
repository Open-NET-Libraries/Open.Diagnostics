...
public class BenchmarkConsoleReport<TBenchParam>
{
	...
	public BenchmarkConsoleReport(uint iterations, TextWriter output, Func<uint, uint, TBenchParam, TimedResult[]> benchmark)
		: this(iterations, benchmark)
	{
		Output = output;
	}

	public BenchmarkConsoleReport(uint iterations, StringBuilder output, Func<uint, uint, TBenchParam, TimedResult[]> benchmark)
		: this(iterations, new StringWriter(output), benchmark)
	{
	}
	...
	protected virtual TimedResult[] BenchmarkResults(uint count, uint repeat, TBenchParam param)
		=> BenchmarkFunction(count, repeat, param);
	...
	public void Test(uint count, uint multiple = 1)
	{
		...
		var results = _benchmarks.ConvertAll(b => TestResult(batch, b.Item1, count, repeat, b.Item2(count)));
